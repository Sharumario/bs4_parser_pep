...
def pep(session):
    ...
    for tr in tqdm(trs[1:]):
        href_pep = tr.td.find_next_sibling().find('a')['href']
        url_pep = urljoin(MAIN_PEP_URL, href_pep)
        pep_statuslist = EXPECTED_STATUS.get(tr.td.text[1:])
        if not pep_statuslist:
            pep_statuslist = tr.td.text[1:]
            logging.info(
                f'"{url_pep}".'
                f'Неизвестный ключ статуса: \'{pep_statuslist}\''
            )
        response = get_response(session, url_pep)
        if response is None:
            return None
        soup = BeautifulSoup(response.text, features='lxml')
        status = soup.find(string='Status').find_next('dd').text
